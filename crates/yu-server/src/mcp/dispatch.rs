...
pub async fn dispatch(state: &AppState, session_id: &str, msg: Value) -> Option<Value> {
    ...
    match method {
        ...
        "tools/call" => {
            ...
            let call_result = call_tool(state, name, arguments).await;
...
async fn call_tool(state: &AppState, name: &str, _args: Value) -> Result<String, String> {
    match name {
        "get_server_info" => Ok(crate::routes::server_info::server_info_body(state).to_string()),
...
mod tests {
    ...
    async fn denial_is_returned_as_result_content_not_a_protocol_error() {
        ...
        let resp = dispatch(&state, "conn-1", tools_call("delete_something", json!({})))
            .await
            .unwrap();
        assert!(
    ...
    async fn allowed_tool_reaches_call_tool() {
        let state = test_state().await;
        let resp = dispatch(&state, "conn-1", tools_call("get_server_info", json!({})))
            .await
            .unwrap();
    ...
    async fn auto_scan_info_matches_rest_shape() {
        let state = test_state().await;
        let resp = dispatch(&state, "conn-1", tools_call("auto_scan_info", json!({})))
            .await
            .unwrap();
        assert!(resp.get("error").is_none());
    ...
    async fn auto_scan_start_returns_400_body_when_no_scan_roots_configured() {
        let state = test_state().await;
        let resp = dispatch(&state, "conn-1", tools_call("auto_scan_start", json!({})))
            .await
            .unwrap();
    ...
    async fn auto_scan_stop_reports_not_running_when_watcher_never_started() {
        let state = test_state().await;
        let resp = dispatch(&state, "conn-1", tools_call("auto_scan_stop", json!({})))
            .await
            .unwrap();
        assert!(resp.get("error").is_none());
    ...
    async fn scope_denial_is_recorded_in_the_action_journal() {
        let state = test_state().await;
        dispatch(&state, "conn-1", tools_call("delete_something", json!({})))
            .await
            .unwrap();
    ...
    async fn successful_tool_execution_is_recorded_in_the_action_journal() {
        let state = test_state().await;
        dispatch(&state, "conn-1", tools_call("get_server_info", json!({})))
            .await
            .unwrap();
        let rows = journal_rows(&state).await;
        assert_eq!(
    ...
    async fn agent_scope_bind_then_reconnect_with_same_agent_id_keeps_scope() {
        ...
        let resp = dispatch(&state, "conn-a", tools_call("delete_something", json!({})))
            .await
            .unwrap();
        assert!(
        ...
        .await
        .unwrap();
        let resp2 = dispatch(&state, "conn-b", tools_call("delete_something", json!({})))
            .await
            .unwrap();
        assert!(
    ...
    async fn auto_scan_start_returns_409_body_when_already_running() {
        ...
        let first = dispatch(&state, "conn-1", tools_call("auto_scan_start", json!({})))
            .await
            .unwrap();
        assert!(
        ...
        let second = dispatch(&state, "conn-1", tools_call("auto_scan_start", json!({})))
            .await
            .unwrap();
