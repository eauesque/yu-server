...
pub(crate) async fn run_tagger_batch_worker(
    ...
    let general_thr = tagger_config["general_threshold"].as_f64().unwrap_or(0.35) as f32;
...
fn local_failure_reason(outcome: &crate::routes::wd_infer::WdInferOutcome) -> &'static str {
    match outcome {
        ...
        crate::routes::wd_infer::WdInferOutcome::Success(_) => {
            unreachable!("success is not a failure")
        }
    }
}
