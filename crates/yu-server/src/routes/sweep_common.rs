...
pub(crate) fn build_itxt_chunk(xmp_xml: &str) -> Vec<u8> {
    // iTXt payload: keyword\0 comp_flag=0 comp_method=0 lang_tag\0 translated_kw\0 text
    let mut payload: Vec<u8> = XMP_KEY.to_vec();
    payload.extend_from_slice(&[0u8, 0, 0, 0, 0]); // \0 flag method lang\0 tkey\0
    payload.extend_from_slice(xmp_xml.as_bytes());

    let mut chunk: Vec<u8> = (payload.len() as u32).to_be_bytes().to_vec();
...
pub(crate) fn write_png_xmp(path: &Path, xmp_xml: &str) -> std::io::Result<()> {
    ...
    let new_chunk = build_itxt_chunk(xmp_xml);
...
pub async fn upsert_files_from_paths(
    ...
    for path_str in paths {
        match native_import_one(&state.db, path_str).await {
            Ok(file_id) => {
...
async fn native_import_one(pool: &SqlitePool, path_str: &str) -> Result<i64, String> {
    ...
    if let Some((file_id, old_mtime, old_size, old_parser_version)) = existing {
        if !should_rescan(old_mtime, old_size, old_parser_version, mtime, size, false) {
            return Ok(file_id);
        }
    }
...
async fn bare_upsert_one(pool: &SqlitePool, path_str: &str) -> Result<i64, String> {
    let p = std::path::Path::new(path_str);
    let meta = p.metadata().map_err(|e| e.to_string())?;
    let mtime = meta
        .modified()
