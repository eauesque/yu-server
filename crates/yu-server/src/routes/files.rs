...
pub(crate) fn bind_pdfium(library_dir: &FsPath, system_fallback: bool) -> Option<Pdfium> {
    let bindings =
        match Pdfium::bind_to_library(Pdfium::pdfium_platform_library_name_at_path(library_dir)) {
...
fn render_pdf_page(
    ...
    let page = document
        ...
        .ok()?;
    let width = (page.width().value * PDF_RENDER_DPI / 72.0).round() as i32;
    let bitmap = page
