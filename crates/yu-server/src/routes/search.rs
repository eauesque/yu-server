...
fn safe_uint(s: Option<&str>, default: u64) -> u64 {
    match s {
        None | Some("") => default,
        Some(v) => v.trim().parse::<i64>().unwrap_or(default as i64).max(0) as u64,
    }
}
...
impl From<SearchQueryRaw> for SearchParams {
    fn from(r: SearchQueryRaw) -> Self {
        ...
        let limit = safe_int(limit_raw, 100).clamp(1, 2000) as u32;
...
pub async fn search_grouped(
    ...
    let total_files: usize = groups
        .iter()
        .map(|g| g["matchCount"].as_u64().unwrap_or(0) as usize)
        .sum();
