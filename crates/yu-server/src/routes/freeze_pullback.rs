...
fn compute_viewport(
    ...
    let crop_h = ((img_h as f64 / scale) as u32).min(img_h);
    let crop_w = ((crop_h as f64 * aspect) as u32).min(img_w);
    let crop_h = crop_h.min(img_h);
    let center_x = (cx * img_w as f64) as i64;
    let center_y = (cy * img_h as f64) as i64;
    let x = (center_x - crop_w as i64 / 2).clamp(0, (img_w.saturating_sub(crop_w)) as i64) as u32;
    let y = (center_y - crop_h as i64 / 2).clamp(0, (img_h.saturating_sub(crop_h)) as i64) as u32;
    Viewport {
        x,
        y,
        w: crop_w,
...
fn build_vignette(w: u32, h: u32) -> Vec<u8> {
    ...
    for step in 0..20u32 {
        ...
        let a = (60.0 * (step as f64 / 20.0).powi(2)) as u8;
        for y in 0..h {
            for x in 0..w {
...
fn apply_vignette(frame: &mut [u8], alpha: &[u8]) {
    for (i, &a) in alpha.iter().enumerate() {
        ...
        let base = i * 3;
        frame[base] = (frame[base] as f64 * factor) as u8;
        frame[base + 1] = (frame[base + 1] as f64 * factor) as u8;
        frame[base + 2] = (frame[base + 2] as f64 * factor) as u8;
    }
}
...
async fn render_video(
    ...
    let hold_frames = (params.hold_seconds * params.fps as f64).round() as u64;
    let pull_frames = (params.pull_seconds * params.fps as f64).round() as u64;
