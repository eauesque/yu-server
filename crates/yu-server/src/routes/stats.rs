...
async fn build_basic_stats(
    ...
    cache
        .get_or_try_insert_with(|| build_basic_stats_uncached(pool))
        .await
}
...
async fn build_timeline_stats(pool: &SqlitePool, granularity: &str) -> Result<Value, sqlx::Error> {
    if !table_exists(pool, "files").await? {
        return Ok(json!({}));
    }
...
async fn build_model_stats(
    ...
    cache
        .get_or_try_insert_with(|| build_model_stats_uncached(pool))
        .await
}
...
async fn build_resolution_stats(pool: &SqlitePool) -> Result<Value, sqlx::Error> {
    if !table_exists(pool, "files").await? {
        return Ok(json!({}));
    }
    let rows = sqlx::query(&format!(
        ...
        if prev_count > 0 {
            ...
            if growth_rate > 0.5 {
                events.push(json!({
                    ...
                    "params": {"percent": format!("{}", (growth_rate * 100.0) as i64)},
                }));
                ...
                events.push(json!({
                    ...
                    "params": {"percent": format!("{}", (growth_rate.abs() * 100.0) as i64)},
                }));
            }
        }
...
async fn build_stats_all(state: &SharedState) -> Result<Value, sqlx::Error> {
    ...
    Ok(json!({
        ...
        "basic": build_basic_stats(pool, &state.stats_basic_cache).await?,
        "hourly": build_hourly_stats(pool).await?,
        "timeline": build_timeline_stats(pool, "month").await?,
        "models": build_model_stats(pool, &state.stats_models_cache).await?,
        "resolutions": build_resolution_stats(pool).await?,
    }))
}
...
pub async fn stats_basic(
    ...
    match build_stats_all(&state).await {
        Ok(value) => {
            let mut basic = value["basic"].clone();
            if value["_stale"].as_bool().unwrap_or(false) {
...
pub async fn stats_timeline(
    ...
    let granularity = params
        ...
        .map(String::as_str)
        .unwrap_or("month");
    match build_timeline_stats(&state.db_read, granularity).await {
        Ok(value) => api_result(value),
        Err(error) => api_error(error),
    }
}
...
pub async fn stats_resolutions(
    ...
    match build_resolution_stats(&state.db_read).await {
        Ok(value) => api_result(value),
        Err(error) => api_error(error),
    }
}
