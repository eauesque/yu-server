...
fn sd_bracket_weaken_to_nai(s: &str) -> String {
    re(r"\[+([^\[\]:]+)\]+")
        .replace_all(s, |caps: &regex::Captures| {
            ...
            let weight = format_weight((1.0 / SD_BASE).powi(depth as i32));
            format!("{weight}::{inner} ::")
        })
        .to_string()
}
...
fn nai_bracket_weaken_to_sd(s: &str) -> String {
    re(r"\[+([^\[\]]+)\]+")
        .replace_all(s, |caps: &regex::Captures| {
            ...
            let depth = full.chars().take_while(|&c| c == '[').count();
            let inner = &caps[1];
            let weight = format_weight((1.0 / NAI_BASE).powi(depth as i32));
            format!("({inner}:{weight})")
        })
        .to_string()
}
...
pub fn analyze_emphasis(prompt: &str) -> Vec<serde_json::Value> {
    ...
    for caps in re(r"\(+([^():]+)\)+").captures_iter(prompt) {
        let full = &caps[0];
        let depth = full.chars().take_while(|&c| c == '(').count();
        let weight = SD_BASE.powi(depth as i32);
        tokens.push(serde_json::json!({
            "text": caps[1].trim(),
    ...
    for caps in re(r"\{+([^{}]+)\}+").captures_iter(prompt) {
        let full = &caps[0];
        let depth = full.chars().take_while(|&c| c == '{').count();
        let weight = NAI_BASE.powi(depth as i32);
        tokens.push(serde_json::json!({
            "text": caps[1].trim(),
            "weight": (weight * 1_000_000.0).round() / 1_000_000.0,
            "syntax": "nai_brace",
    ...
    for caps in re(r"\[+([^\[\]:]+)\]+").captures_iter(prompt) {
        let full = &caps[0];
        let depth = full.chars().take_while(|&c| c == '[').count();
        let weight = (1.0 / SD_BASE).powi(depth as i32);
        tokens.push(serde_json::json!({
            "text": caps[1].trim(),
            "weight": (weight * 1_000_000.0).round() / 1_000_000.0,
