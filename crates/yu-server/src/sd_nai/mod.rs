...
fn convert_bracket_weaken_to_nai(text: &str) -> String {
    SD_BRACKET_WEAK_RE
        .replace_all(text, |caps: &Captures| {
            let full = &caps[0];
            let depth = full.chars().take_while(|ch| *ch == '[').count() as i32;
            let inner = &full[depth as usize..full.len() - depth as usize];
            format!(
                "{}::{}::",
                w((1.0 / SD_BASE).powi(depth)),
                safe_close(inner)
            )
        })
        .into_owned()
...
fn convert_bracket_weaken_to_sd(text: &str) -> String {
    NAI_BRACKET_WEAK_RE
        .replace_all(text, |caps: &Captures| {
            ...
            let depth = full.chars().take_while(|ch| *ch == '[').count() as i32;
            let inner = &full[depth as usize..full.len() - depth as usize];
            format!("({}:{})", inner, w((1.0 / NAI_BASE).powi(depth)))
        })
        .into_owned()
}
